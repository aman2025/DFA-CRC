...
import { Button, ConfigProvider, Dialog, Field, Form, Input, Loading, Message } from '@alifd/next';
...
class EditorNameSpace extends React.Component {
  ...
  openDialog(record, datasource) {
    this.getNamespaceDetail(record);
    this.setState({
      dialogvisible: true,
      type: record.type,
      datasource: this.otherNameSpaces(record, datasource)
    });
	}

	otherNameSpaces(record, datasource) {
		return datasource.filter(val => val.namespaceShowName !== record.namespaceShowName);
	}
    ...
    this.field.validate((errors, values) => {
      if (errors) {
        return;
			...
			// 判断命名空间名称是否存在
			const flag = this.state.datasource.some(
